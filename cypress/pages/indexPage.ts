...
class IndexPage {
  ...
  validateLoginSucceeded() {
    cy.fixture("userDataLogin.json").then((userdata) => {
      cy.get(loginModalLocators.usernameInput).type(userdata.username, {
        force: true,
      });
      cy.get(loginModalLocators.passwordInput).type(userdata.password, {
        force: true,
      });
  ...
  validateContactMsgIsSent() {
    ...
    cy.fixture("dataContact.json").then((data) => {
      cy.get(indexPageLocators.contactEmailInput).type(data.contactEmail, {
        force: true,
      });
      cy.get(indexPageLocators.contactNameInput).type(data.contactName, {
        force: true,
      });
      cy.get(indexPageLocators.messageInput).type(data.message, {
        force: true,
      });
    });
